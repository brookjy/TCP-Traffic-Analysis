...
#define MAX_NUM_CONNECTION 4000
...
/* The struction from the Lab */
struct connection{
	char src[MAX_STR_LEN]; /*source ip*/
	char dst[MAX_STR_LEN]; /*destination ip*/
	uint16_t port_s; /*source port number*/
	uint16_t port_d; /*destination port number*/
	int  tflags;
      ...
      unsigned int capture_len)
{
	...
	/* Get all the information from the tcp header and store it to the list -> cList[]*/
	...
	connect.port_s=ntohs(tcp->th_sport);
	connect.port_d=ntohs(tcp->th_dport);
	strcpy(cList[total].src, connect.src);
	strcpy(cList[total].dst, connect.dst);
	cList[total].port_s = connect.port_s;
	cList[total].port_d = connect.port_d;
...
void buildFilter(struct built *actual, struct connection *cList, int total){
	...
	for (j=0; j<total;j++){
		if(cList[j].is_set2 == 0){
			...
			actual1[0].port_s = cList[j].port_s;
			actual1[0].port_d = cList[j].port_d;
			cList[j].is_set2 = 1;
			count++;
		}
		k=1;
		int i=j+1;
		for (;i<total;i++){
			if ((!strcmp(cList[i].src,actual1[0].src) && !strcmp(cList[i].dst,actual1[0].dst)
				&& actual1[0].port_s == cList[i].port_s && actual1[0].port_d == cList[i].port_d 
				&& cList[i].is_set2 == 0) || (!strcmp(cList[i].src,actual1[0].dst) && 
				!strcmp(cList[i].dst,actual1[0].src) && actual1[0].port_s == cList[i].port_d 
				&& actual1[0].port_d == cList[i].port_s && cList[i].is_set2 == 0)) {

				cList[i].is_set2 = 1;
			    strcpy(actual1[k].src, cList[i].src);
				strcpy(actual1[k].dst, cList[i].dst);
				actual1[k].port_s = cList[i].port_s;
				actual1[k].port_d = cList[i].port_d;
				k++;
...
int checkConn(struct built *actual, struct connection *cList, struct timeval ts,int total, int print )
{
	/* Initialization of all the variables*/
	...
	int j;
	int k;
	int nums;int num1;int k1;int k2;
	int constant;
	int only = 1;
	int many = 1;
	...
	for (j=0; j<total;j++){
		...
		if(cList[j].is_set == 0){
			...
			actual[0].port_s = cList[j].port_s;
			actual[0].port_d = cList[j].port_d;
			...
			if (cList[j].tflags == 17 || cList[j].tflags == 1 || cList[j].tflags == 25){
    			countf++;
			...
			if ((cList[j].tflags == 4 && only == 1) || (only == 1 && cList[j].tflags == 20 )){
				countr++;
				only = 0;
		...
		int i=j+1;
		for (;i<total;i++){
			if ((!strcmp(cList[i].src,actual[0].src) && !strcmp(cList[i].dst,actual[0].dst)
				&& actual[0].port_s == cList[i].port_s && actual[0].port_d == cList[i].port_d 
				&& cList[i].is_set == 0) || (!strcmp(cList[i].src,actual[0].dst) && 
				!strcmp(cList[i].dst,actual[0].src) && actual[0].port_s == cList[i].port_d 
				&& actual[0].port_d == cList[i].port_s && cList[i].is_set == 0)) {

				cList[i].is_set = 1;
				
			    strcpy(actual[k].src, cList[i].src);
				strcpy(actual[k].dst, cList[i].dst);
				actual[k].port_s = cList[i].port_s;
				actual[k].port_d = cList[i].port_d;
				...
				if ((cList[i].tflags == 4 && only == 1) || (only == 1 && cList[i].tflags == 20 )){
					countr++;
					only = 0;
				}
				if (cList[i].tflags == 17 || cList[i].tflags == 1 || cList[i].tflags == 25){
    				countf++; 
		...
		while (k>0){
			if(!strcmp(actual[k].src,actual[0].src) && !strcmp(actual[k].dst,actual[0].dst)
				&& actual[0].port_s == actual[k].port_s && actual[0].port_d == actual[k].port_d 
				){
				checkAll.num_packet_src++;
				sdata_len = sdata_len + actual[k].length;

			}
			if(!strcmp(actual[k].src,actual[0].dst) && !strcmp(actual[k].dst,actual[0].src) 
				&& actual[0].port_s == actual[k].port_d && actual[0].port_d == actual[k].port_s 
				) {
		...
		if ((counts == 1 && countf == 1) || (counts == 2 &&
			countf == 1) || (counts == 2 && countf == 2)){
				connected++;		
				print = 3;
		...
		if (print == 3){
			...
			printf("Duration: %f\n",(actual[num1].started - cList[0].started)-(actual[0].started - cList[0].started));
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	printf("Output format of project2: \n\n");
